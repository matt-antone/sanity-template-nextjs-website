...
const PaintList: React.FunctionComponent<IPaintListProps> = ({ paintList = [] }) => {
  const possibleColors = ["#000", "#fff"];

  return paintList?.length > 0 &&(
    <ul className="list-none text-sm grid grid-cols-2 divide-x">
      {paintList.map((paint: any) => {
        ...
        return (
          <li key={paint._key} className="flex flex-col gap-2 p-4">
            ...
            <div>
              <ul className="flex flex-col w-auto pt-4">
                {paint.mix.map((p: any, index: number) => {
                  const margin = index % 2 === 0 ? paint.mix.length === 1 ? "" : "mr-10" : paint.mix.length === 1 ? "" : "ml-10";
                  const textColor = bestContrast(p.paint.color?.hex || "#000", possibleColors);
                  return (
                    <li
                      key={p._key}
                    ...
                    >
                      ...
                      >
                        { p.paint.color ? <Hexagon
                          className={`w-12 h-12`}
                          style={{
                            fill: p.paint.color?.hex || "#000",
                          }}
                        /> : <HexagonOutline className={`w-12 h-12 fill-slate-300`} />}
                      ...
                      <span
                        className={`flex pl-4`}
                      >
                        <span><span className="font-bold">{p.paint.title}</span> ({p.parts} {`part${p.parts > 1 ? "s" : ""}`})</span>
                      </span>
                    </li>
                  );
                })}
